...
export const diagnoseSupabaseConfig = createServerFn({ method: "GET" }).handler(
  async () => {
    ...
    return {
      ...
      decodedRole: keyRole,
      envKeys: Object.keys(process.env).filter(k => k.toLowerCase().includes("supabase") || k.toLowerCase().includes("service") || k.toLowerCase().includes("razorpay") || k.toLowerCase().includes("resend"))
    };
  }
);
