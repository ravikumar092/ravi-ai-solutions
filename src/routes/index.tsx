...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "Ravi Kumar AI Lab — Build Smarter. Automate More. Earn Online." },
      ...
      { property: "og:title", content: "Ravi Kumar AI Lab — Build Smarter. Automate More. Earn Online." },
...
function Landing() {
  ...
  useEffect(() => {
    if (settings?.site_name) {
      document.title = `${settings.site_name} — Build Smarter. Automate More. Earn Online.`;
    }
  ...
  return (
    <div className="min-h-screen bg-background text-foreground selection:bg-primary/20">
      ...
      <main>
        {/* Hero Section */}
        <Hero settings={settings} onBook={() => { setSelectedService(""); setModal(true); }} />
        ...
        <Tutorials settings={settings} videos={videos ?? []} />
...
/* ── HERO ── */
function Hero({ settings, onBook }: { settings: any; onBook: () => void }) {
  const r = useReveal<HTMLDivElement>();

  useEffect(() => { r.current?.classList.add("in"); }, [r]);

  const tagline = settings?.hero_tagline || "I design and ship AI workflows, agentic pipelines, and custom automation using n8n, Make, LangChain, CrewAI, and more — tailored to your business.";
  const headline = settings?.hero_headline || "Build autonomous systems that work while you sleep.";
  const headlineWords = headline.split(" ");
  const highlightCount = Math.min(Math.ceil(headlineWords.length * 0.4), 4);
  const mainHeadline = headlineWords.slice(0, -highlightCount).join(" ");
  const highlightHeadline = headlineWords.slice(-highlightCount).join(" ");
    ...
    <section className="relative pt-40 pb-20 overflow-hidden">
      ...
      <div ref={r} className="reveal relative mx-auto max-w-5xl px-6 text-center">
        ...
        <h1 className="font-display text-5xl md:text-[4.75rem] font-bold leading-[1.03] tracking-[-0.03em] mb-8">
          {mainHeadline}{" "}
          <span className="neon-text font-black">{highlightHeadline}</span>
        </h1>
...
const formatPriceDisplay = (price: string | null, currency: string = "USD") => {
  if (!price) return "";
                  ...
                  <div className="mt-auto pt-6 border-t border-border/50 flex items-end justify-between gap-4 mt-6">
                    <div>
                      <p className={`font-display text-3xl font-bold ${isPopular ? "neon-text" : "text-foreground"}`}>
                        {formatPriceDisplay(s.price || s.price_label, s.currency)}
                      </p>
...
/* ── LEAD CAPTURE FUNNEL ── */
function LeadCapture({ settings }: { settings: any }) {
  ...
  const ebookTitle = settings?.ebook_title || "Ravi Kumar AI Lab Playbook";
  const ebookDesc = settings?.ebook_desc || "Step-by-step blueprints, stack suggestions, checklists, and 50+ micro-niche ideas to launch your solo business and hit $10k MRR. Over 3,500 builders have downloaded it.";
  ...
  const handleSubscribe = async (e: React.FormEvent) => {
    ...
    try {
      await submit({
        ...
        goal: `Download ${ebookTitle}`,
        stage: "Waitlist / Lead",
        needs: "Subscribed via landing page Free Ebook form.",
        best_time: "",
      });
      toast.success(`Check your inbox! We've sent your copy of ${ebookTitle}.`);
      setEmail("");
      setFirstName("");
    } catch (err: any) {
        ...
        <div className="bg-card/40 border border-border rounded-2xl p-8 md:p-12 backdrop-blur text-center space-y-6">
          ...
          <div className="max-w-xl mx-auto space-y-3">
            <h2 className="font-display text-3xl font-bold tracking-tight">
              Get the <span className="neon-text">{ebookTitle}</span> (Free)
            </h2>
            <p className="text-xs text-muted-foreground leading-relaxed">
              {ebookDesc}
            </p>
...
/* ── TUTORIALS ── */
function Tutorials({ settings, videos }: { settings: any; videos: any[] }) {
  const r = useReveal<HTMLDivElement>();
      ...
      <div ref={r} className="reveal mx-auto max-w-6xl px-6 grid md:grid-cols-2 gap-20 items-center">
        <div>
          ...
          <p className="text-muted-foreground leading-relaxed text-[15px] mb-8">
            Every pipeline, n8n schema, and agent layout showcased here is derived directly from client setups or co-building workshops.
          </p>
        ...
        >
          ...
          <div className="absolute bottom-5 left-6 right-6">
            <p className="font-display text-base font-semibold">{siteName} channel</p>
            <p className="text-xs text-muted-foreground mt-0.5">By {founderName} — AI automation blueprints</p>
          </div>
        </a>
      </div>
    </section>
...
/* ── SCHEDULE ── */
function Schedule({ settings }: { settings: any }) {
  const r = useReveal<HTMLDivElement>();
  const calendlyUrl = settings?.calendly_url || "https://calendly.com/ravikumar-devforge";
  ...
  const formattedCalendlyUrl = calendlyUrl.includes("?") 
    ? calendlyUrl 
    : `${calendlyUrl}?background_color=181a1f&text_color=f5f7fa&primary_color=4ade80`;
  ...
  return (
    <section id="schedule" className="py-24 border-t border-border">
      <div ref={r} className="reveal mx-auto max-w-5xl px-6">
        ...
        <div key={calendlyUrl} className="rounded-xl border border-border overflow-hidden bg-card/30">
          <div
            className="calendly-inline-widget"
            data-url={formattedCalendlyUrl}
